...
const Requests = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://irequest.rumlyn.com/api/v1/processed-requests/all",
      requestOptions
    )
  ...
  const handleDelete = (id) => {
    ...
    fetch(
      `https://irequest.rumlyn.com/api/v1/processed-requests/${id}`,
      requestOptions
    )
      .then((response) => {
        if (response.ok) {
  ...
  const customSStatuses = async () => {
    try {
      ...
      fetch("https://irequest.rumlyn.com/api/v1/statuses/all", requestOptions)
        .then((response) => response.json())
        .then((result) => {
    ...
    <div className="min-h-screen py-8 px-4 md:px-12">
                  ...
                  <select name="" id="">
                    {statuses.map((status) => (
                      <option
                        ...
                        value={status.status}
                        selected={
                          statusToEdit === status.status? true : false
                        }
                      >
                        {status.name}
                      </option>
                ...
                <td className="py-3 px-6 text-center space-x-2 flex items-center">
                  {editableRowId === request._id ? (
                    <>
                      <button
                        onClick={() =>
                          handleStatusChange(request._id, statusToEdit)
                        }
      ...
      {selectedRequest && (
        ...
        >
          ...
          <div className="mb-4">
            <p>
              <strong className="font-semibold">Request Code:</strong>{" "}
            ...
            <p>
              <strong className="font-semibold">Date Created:</strong>{" "}
              {new Date(selectedRequest.dateCreated).toLocaleString()}
            </p>
            <p>
              <strong className="font-semibold">Date Updated:</strong>{" "}
              {new Date(selectedRequest.dateUpdated).toLocaleString()}
            </p>
          </div>
        </animated.div>
      )}
